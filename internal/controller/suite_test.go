...
var _ = BeforeSuite(func() {
	...
	var err error
	err = snaplanev1alpha1.AddToScheme(scheme.Scheme)
	...
	if snapshotCRDPath := findExternalSnapshotterCRDPath(); snapshotCRDPath != "" {
		crdPaths = append(crdPaths, snapshotCRDPath)
		By("using external snapshotter CRDs from " + snapshotCRDPath)
	}
	testEnv = &envtest.Environment{
	...
	// Prefer explicitly configured envtest assets path.
	...
	} else if getFirstFoundEnvTestBinaryDir() != "" {
...
func findExternalSnapshotterCRDPath() string {
	...
	pattern := filepath.Join(modCache, "github.com", "kubernetes-csi", "external-snapshotter", "client", "v8@*", "config", "crd")
